...
arguments = [generateFrequencies(6)]

for filterType, cutoffFreq in zip(['lowpass', 'highpass'], ['50','100']):
    dt = '0.00001'
    valueContents = []
    eventContents = []
    populationIDs = []
    for arg in arguments:
        output = check_output(['../../../build/Test/Population_Latency_Coding_Sine_Lowpass_Results', *arg, '0.0003', '200', dt, filterType + ',' + cutoffFreq])
    ...
    for valueContent, populationID, args in zip(valueContents, populationIDs, arguments):
        ...
        ax1.set_xlim([0, 0.5])
        ax1.set_title('Population latency coding, 200 neurons \n ' + filterType.capitalize() + ' filtering, cutoff=' + cutoffFreq + 'Hz \n Signal frequencies = [' + ','.join(args) + ']Hz', fontsize='x-large', fontweight='bold')
        ...
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'population_latency_coding_' + filterType + '_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')

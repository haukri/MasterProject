...
for dt, cutoffFreq in zip(['0.00001'], ['50']):
    ...
    for arg in arguments:
        output = check_output(['../../../build/Test/Latency_Coding_Sine_Lowpass_Results', *arg, '0.0003', dt, 'lowpass,' + cutoffFreq])
    ...
    for valueContent, populationID, args in zip(valueContents, populationIDs, arguments):
        ...
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'latency_coding_lowpass_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')

...
for filterType, cutoffFreq in zip(['lowpass', 'highpass'], ['2','3']):
    ...
    for arg in arguments:
        output = check_output(['../../../build/Test/Population_Latency_Coding_Lowpass_Example_Results', *arg, '0.01', '30', dt, filterType + ',' + cutoffFreq])
    ...
    for valueContent, populationID, args in zip(valueContents, populationIDs, arguments):
        ...
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'population_latency_coding_' + filterType + '_example_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')

...
for dt, cutoffFreq in zip(['0.0001'], ['2']):
    ...
    for valueContent, populationID, args in zip(valueContents, populationIDs, arguments):
        ...
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'population_latency_coding_lowpass_example_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')

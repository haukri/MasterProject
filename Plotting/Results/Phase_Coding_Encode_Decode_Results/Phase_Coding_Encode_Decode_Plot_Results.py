...
for dt, window in zip(['0.001', '0.0001', '0.00001'], ['0.03', '0.003', '0.0003']):
    ...
    for valueContent, populationID, args in zip(valueContents, populationIDs, arguments):
        ...
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'phase_coding_plot_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, marker='pdf')

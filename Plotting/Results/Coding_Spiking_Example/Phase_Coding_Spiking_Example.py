...
for dt, cutoffFreq in zip(['0.0001'], ['2']):
    ...
    for arg in arguments:
        output = check_output(['../../../build/Test/Phase_Coding_Lowpass_Example_Results', *arg, '0.01', '1000', dt, 'lowpass,' + cutoffFreq])
    ...
    for valueContent, eventContent, populationID, args in zip(valueContents, eventContents, populationIDs, arguments):
        ...
        ax2.set_title('Membrane potential of spiking neuron', fontsize='x-large', fontweight='bold')
        ...
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'phase_coding_spiking_example_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')

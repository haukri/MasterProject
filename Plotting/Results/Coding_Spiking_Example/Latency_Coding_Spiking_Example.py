...
for dt, cutoffFreq in zip(['0.0001'], ['2']):
    ...
    for valueContent, eventContent, populationID, args in zip(valueContents, eventContents, populationIDs, arguments):
        ...
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'latency_coding_spiking_example_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')

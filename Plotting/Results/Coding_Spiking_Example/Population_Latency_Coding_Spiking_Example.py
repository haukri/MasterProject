...
def plotSpikeOutputs(ax, eventContent, populationID, neuronID, color='blue', neuronOffset=0):
    ...
    ax.eventplot(x, lineoffsets=-70, linelengths=60, color='red', linewidths=1)
...
for dt, cutoffFreq in zip(['0.0001'], ['2']):
    ...
    for arg in arguments:
        output = check_output(['../../../build/Test/Population_Latency_Coding_Lowpass_Example_Results', *arg, '0.01', '30', dt, 'lowpass,' + cutoffFreq])
        populationIDs.append([output.decode('UTF-8').split('\n')[0], output.decode('UTF-8').split('\n')[1], output.decode('UTF-8').split('\n')[2]])
    ...
    for valueContent, eventContent, populationID, args in zip(valueContents, eventContents, populationIDs, arguments):
        ...
        ax2.set_title('Membrane potential of spiking neuron 0', fontsize='x-large', fontweight='bold')
        ax2.set_xlabel('Time [s]', fontsize='x-large', fontweight='bold')
        ax2.set_ylabel('Membrane potential [mV]', fontsize='x-large', fontweight='bold')
        ...
        plotSpikeOutputs(ax2, eventContent, populationID[2], '0')
        plotValueOutputs(ax2, valueContent, populationID[2], '0', '0', label='Membrane')
        # ------------- Plot Setup ------------- #
        plotValueOutputs(ax1, valueContent, populationID[0], '0', '3', label='Decoded signal')
        plotValueOutputs(ax1, valueContent, '999', '0', '3', label='Original signal')
        # plotValueOutputs(ax1, valueContent, '999', '0', '3', 'Test signal 1')
        ax1.legend(prop=dict(weight='bold', size='large'), loc='lower right')
        plt.tight_layout()
        if saveFigure:
            plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/../figures/' + 'population_latency_coding_spiking_example_' + dt.replace('.', '_') + '_' + args[-1] + '.pdf', bbox_inches='tight', dpi=300, format='pdf')
